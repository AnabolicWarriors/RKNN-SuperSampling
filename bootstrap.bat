...
docker run -d -p 8000:22 -v %cd%:/workspaces -e PATH=/workspaces/components/gcc-linaro-6.3.1-2017.02-x86_64_aarch64-linux-gnu/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin -e LD_LIBRARY_PATH=./lib/ --name i_love_chacha aoikazto/sshd:ubuntu20.04
